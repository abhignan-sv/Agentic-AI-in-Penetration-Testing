...
recon_agent = Agent(
    ...
    goal='Identify all active hosts, open ports, and running services on the target network, '
         'and gather initial web application information. Prioritize stealth if IDS/IPS is detected. '
         'If you discover a Damn Vulnerable Web Application (DVWA), use the dvwa_authenticated_scan tool to perform a thorough scan using the default credentials (admin/password)'
         'Perform comprehensive web application scanning including directory brute-forcing. '
         'For any discovered web application, attempt to determine its type and use appropriate scanning methods. '
         'Use advanced tools like gobuster, dirb, and nikto for thorough assessment. '
         'Consult Google Gemini AI for optimal scanning strategies and give final report.',
         
    backstory=(
        "You are an expert in passive and active reconnaissance. Your mission is to meticulously "
        "map out the target's digital footprint, providing crucial initial data for the pen-test team. "
        "You are thorough, accurate, and can adapt your scanning techniques if defensive measures are present."
    ),
    tools=[nmap_scan, dvwa_authenticated_scan, web_vulnerability_scan, gobuster_scan, dirb_scan, 
           ids_detection_tool, gemini_consultation, retrieve_from_long_term_memory],
...
# Agent 5: Penetration Test Reporter
...
# Definining the target IP/URL 
TARGET_IP = os.getenv("TARGET_IP", "192.168.56.30") 
TARGET_URL = os.getenv("TARGET_URL", f"http://{TARGET_IP}/dvwa") 
INFRASTRUCTURE_TARGET = os.getenv("INFRASTRUCTURE_TARGET", "192.168.56.20")

# Task 1: Reconnaissance
recon_task = Task(
    description=(
        f"Perform a comprehensive reconnaissance on the target IP {TARGET_IP} and URL {TARGET_URL}. "
        "First, use the 'ids_detection_tool' to check for any active defensive measures. "
        "Based on the IDS detection, adjust your Nmap scanning strategy (e.g., slower timing if detected). "
        "Then, use 'nmap_scan' for network and OS detection, and 'web_vulnerability_scan' for initial web app insights. "
        "For any discovered web services, use 'gobuster_scan' and 'dirb_scan' for directory enumeration. "
        "Consult Google Gemini AI for optimal scanning approaches and give final answer."
        "Provide a detailed summary of findings, including open ports, services, versions, OS, and initial web vulnerabilities. "
        "Store key findings in long-term memory using 'add_to_long_term_memory' for future reference."
    ),
    expected_output=(
        "A detailed markdown formatted report of the reconnaissance findings, including "
        "IDS/IPS detection status, Nmap scan results (open ports, services, versions, OS), "
        "and initial web scan findings. Highlight any unusual or potentially vulnerable services. "
        "Confirm that key findings have been added to long-term memory."
    ),
    agent=recon_agent,
    output_file='reports/recon_report.md' 
)
...
vuln_analysis_task = Task(
    description=(
        f"Analyze the reconnaissance data for {TARGET_IP} and {TARGET_URL} to identify specific vulnerabilities. "
...
exploitation_task = Task(
    description=(
        f"Based on the vulnerability analysis for {TARGET_IP} and {TARGET_URL}, attempt to exploit the most critical "
...
post_exploitation_task = Task(
    description=(
        f"You are now on a compromised system ({TARGET_IP}). "
...
final_report_task = Task(
    description=(
        f"Retrieve ALL findings from the database for the penetration test on {TARGET_IP} and {TARGET_URL} "
    ...
    context=[recon_task, vuln_analysis_task, exploitation_task, post_exploitation_task], 
...
pen_test_crew = Crew(
    agents=[recon_agent, vuln_analysis_agent, exploitation_agent, post_exploit_agent, reporting_agent],
    tasks=[recon_task, vuln_analysis_task, exploitation_task, post_exploitation_task, final_report_task],
